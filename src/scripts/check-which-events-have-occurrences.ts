...
async function checkWhichEventsHaveOccurrences() {
  ...
  console.log(`\n📋 All Events (${allEvents.length} total):\n`);
  ...
  console.log(`✅ Events WITH occurrences (${withOccurrences.length}):`);
  for (const event of withOccurrences) {
    console.log(
      `  ${event.name}: ${event._count.occurrences} occ | ` +
  ...
  console.log(`\n❌ Events WITHOUT occurrences (${withoutOccurrences.length}):`);
  ...
  console.log(`\n🌞 SOLAR Rule Events (${solarEvents.length}):`);
  ...
  console.log(`\n🌙 TITHI Rule Events (${tithiEvents.length}):`);
