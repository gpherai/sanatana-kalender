  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (i + 2 < lines.length) {
      ...
      if (date && date.includes(",") && date.includes("2025")) {
        events.push({ name: name || "", date, info: info || "", month: currentMonth });
...
async function compareEvents() {
  ...
  console.log(
    `   Missing:       ${categories.ekadashi.length - ourEkadashiCount} events`
  );
  ...
  majorFestivalNames.forEach((festivalName) => {
    ...
    const inOurs = ourEvents.some((e) => e.name.toLowerCase().includes(festivalName));

    const status = inOurs ? "✅" : "❌";
    console.log(
      `   ${status} ${festivalName.padEnd(20)} - ${inDrik ? "In Drik" : "Not in Drik"}, ${inOurs ? "In DB" : "NOT in DB"}`
    );
  });
  ...
  const sampleMissing = drikEvents
    .filter((drik) => {
      ...
      if (drik.name.includes("Purnima") && drik.name.split(" ").length === 2)
        return false;

      // Check if we have something similar
      return !ourEvents.some(
        (our) =>
  ...
  console.log(
    `   Approximate gap:        ~${drikEvents.length - ourEvents.length} events`
  );
