...
:root {
  ...
  /* Almanac - Day header background (DayDetailsPanel selected day header) */
  --theme-almanac-day-header-bg: linear-gradient(135deg, color-mix(in oklch, var(--theme-primary) 65%, black), var(--theme-secondary));

  /* Almanac - MoonPhasesTimeline background */
  --theme-almanac-moon-timeline-bg: linear-gradient(to right, oklch(0.93 0.04 240), oklch(0.93 0.03 285), oklch(0.94 0.04 330));

  /* Almanac - Event time badges (semantically correct, separate from moon/special category vars) */
  --theme-almanac-event-time-start-bg: oklch(0.88 0.08 240);
  --theme-almanac-event-time-start-fg: oklch(0.42 0.12 245);
  --theme-almanac-event-time-end-bg: oklch(0.92 0.06 75);
  --theme-almanac-event-time-end-fg: oklch(0.42 0.14 65);

  /* Panchanga detail cards (yoga, karana) — default neutral purples */
  --theme-panchanga-yoga-bg: oklch(0.90 0.05 280);
...
/* MoonPhasesTimeline base style — background driven by theme var */
.almanac-moon-timeline {
  background: var(--theme-almanac-moon-timeline-bg);
}
`;
}
...
.dark {
  ...
  /* Almanac - Day header (dark mode — same gradient, primary/secondary already adapt) */
  --theme-almanac-day-header-bg: linear-gradient(135deg, color-mix(in oklch, var(--theme-primary) 65%, black), var(--theme-secondary));

  /* Almanac - MoonPhasesTimeline (dark mode) */
  --theme-almanac-moon-timeline-bg: linear-gradient(to right, oklch(0.18 0.08 240 / 0.25), oklch(0.16 0.06 285 / 0.25), oklch(0.18 0.05 330 / 0.25));

  /* Almanac - Event time badges (dark mode) */
  --theme-almanac-event-time-start-bg: oklch(0.35 0.12 245 / 0.3);
  --theme-almanac-event-time-start-fg: oklch(0.75 0.10 245);
  --theme-almanac-event-time-end-bg: oklch(0.38 0.10 75 / 0.3);
  --theme-almanac-event-time-end-fg: oklch(0.75 0.12 70);

  /* Panchanga detail cards (dark mode defaults) */
  --theme-panchanga-yoga-bg: oklch(0.28 0.08 280 / 0.4);
  --theme-panchanga-yoga-label: oklch(0.75 0.10 278);
  --theme-panchanga-karana-bg: oklch(0.28 0.06 295 / 0.4);
  --theme-panchanga-karana-label: oklch(0.75 0.10 290);
}
`;
}
...
function generateSpecialTheme(theme: ThemeDefinition): string {
  ...
  parts.push(`
/* ═══════════════════════════════════════════════════════════════════════════
   ✨ SPECIAL THEME: ${theme.displayName}
   ${theme.description}
   ═══════════════════════════════════════════════════════════════════════════ */
...
function main(): void {
  ...
  for (const t of standard) {
    ...
    console.log(`    • ${t.displayName}${marker}`);
  }
  ...
  console.log("\n✅ Done!\n");
  console.log(
    "⚠️  Do not manually edit globals.css - regenerate with 'npm run generate:css'\n"
  );
}
