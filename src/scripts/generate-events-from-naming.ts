...
async function generateEventsFromNaming() {
  ...
  for (const naming of EVENT_NAMING_CATALOG) {
    ...
    // fall back to name for events created before namingKey was introduced.
    const existing =
      (await prisma.event.findFirst({ where: { namingKey: naming.key } })) ??
      (await prisma.event.findFirst({ where: { name: naming.name } }));
