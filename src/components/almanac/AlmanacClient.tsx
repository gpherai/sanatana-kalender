...
interface AlmanacClientProps {
  initialMonthData?: DailyInfoResponse[];
  initialMonthEvents?: CalendarEventResponse[];
}
...
export function AlmanacClient({
  ...
  // Seed session cache with SSR-provided initial data (runs once — before useFetch)
  const cacheSeededRef = useRef<true | null>(null);
  if (cacheSeededRef.current == null) {
    cacheSeededRef.current = true;
    const key = `${year}-${month}`;
    if (initialMonthData && !monthDataCache.has(key))
      monthDataCache.set(key, initialMonthData);
    if (initialMonthEvents && !eventsDataCache.has(key))
      eventsDataCache.set(key, initialMonthEvents);
  }
        ...
        <div className="flex h-64 items-center justify-center">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <button
              type="button"
              onClick={handleRetry}
              className="bg-theme-primary focus-visible:ring-theme-primary text-theme-primary-fg hover:bg-theme-primary/80 mt-1 cursor-pointer rounded-lg px-4 py-2 text-sm transition-colors focus-visible:ring-2 focus-visible:outline-none"
            >
              Opnieuw proberen
            </button>
          </div>
