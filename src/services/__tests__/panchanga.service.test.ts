...
describe("Panchanga Service (with Caching)", () => {
  ...
  it("should call Swiss Service for fresh data", async () => {
    ...
    const result = await panchangaService.calculateDaily(
      date,
      MOCK_LOCATION,
      "Europe/Amsterdam"
    );
    ...
    expect(mockComputeDaily).toHaveBeenCalledWith(
      "2025-01-01",
  ...
  it("should return cached data on second call (Caching check)", async () => {
    ...
    const result2 = await panchangaService.calculateDaily(
      date,
      MOCK_LOCATION,
      "Europe/Amsterdam"
    );
  ...
  it("should handle date ranges correctly", async () => {
    ...
    const results = await panchangaService.calculateRange(
      start,
      end,
      MOCK_LOCATION,
      "Europe/Amsterdam"
