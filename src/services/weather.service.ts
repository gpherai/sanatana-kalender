...
function mapCurrent(w: CurrentResponse) {
  const dewPoint = w.main.temp - (100 - w.main.humidity) * 0.2;
  return {
    coord: w.coord,
    dt: w.dt,
...
export async function getWeatherDashboard(
  ...
  if (!weatherRes.ok || !forecastRes.ok) {
    ...
    if (failed.status === 401) {
      throw new WeatherServiceError(
        "invalid_api_key",
        "Ongeldige OpenWeather API sleutel",
        503
      );
    }
    throw new WeatherServiceError("upstream", `OpenWeather fout: ${failed.status}`);
  }
