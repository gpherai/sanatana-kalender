...
async function verifyOffset() {
  ...
  console.log(
    "  2025-01-03 Friday    = Pausha, Shukla Chaturthi (Vighneshvara Chaturthi)"
  );
  console.log("=".repeat(80));
  console.log();
  ...
  dates.forEach((d) => {
    ...
    console.log(
      `${dateStr} ${dayName.padEnd(9)} = ${d.maasName}, ${d.paksha} ${d.tithi}`
    );

    // Check for the smoking gun
    if (dateStr === "2024-12-31" && d.tithi === "PRATIPADA_SHUKLA") {
      console.log(
        "  ⚠️  ^ THIS IS THE SMOKING GUN! Pratipada on 31 Dec instead of 1 Jan"
      );
    }
  ...
  if (jan03?.tithi === "CHATURTHI_SHUKLA") {
    console.log(
      "✅ CONFIRMED: We have Chaturthi on 2025-01-03 (correct tithi for that date)"
    );
