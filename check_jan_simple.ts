...
async function check() {
  ...
  for (const important of importantDates) {
    ...
    if (ourData) {
      ...
      const tithiMatch =
        (important.drikTithi.includes("Ekadashi") &&
          ourData.tithi?.includes("EKADASHI")) ||
        (important.drikTithi.includes("Purnima") && ourData.tithi === "PURNIMA") ||
        (important.drikTithi.includes("Chaturthi") &&
          ourData.tithi?.includes("CHATURTHI"));
  ...
  const chaturthiDates = dailyInfo.filter(
    (d) => d.tithi?.includes("CHATURTHI") && d.paksha === "KRISHNA"
  );
